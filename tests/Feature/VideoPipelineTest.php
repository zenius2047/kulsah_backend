...
class VideoPipelineTest extends TestCase
{
    ...
    public function test_creator_can_queue_ffmpeg_video_edit_with_text_and_drawing_overlays(): void
    {
        config()->set('logging.default', 'null');
        $diskRoot = sys_get_temp_dir().DIRECTORY_SEPARATOR.'kulsah-video-edit-tests';
        File::ensureDirectoryExists($diskRoot);
        ...
        $response = $this
            ...
            ->post("/api/v1/creator/videos/{$video->id}/edits", [
                ...
                'drawing_files' => [
                    UploadedFile::fake()->image('drawing.png', 320, 320),
                ],
            ]);
        ...
        $response->assertAccepted()
            ...
            ->assertJsonPath('data.metadata.edit_overlays.0.text', 'Hello Kulsah')
            ->assertJsonPath('data.metadata.edit_overlays.1.type', 'drawing');

        $video->refresh();

        $this->assertSame('processing', $video->status);
        $this->assertNull($video->cdn_url);
        $this->assertSame('videos/originals/1/source.mp4', $video->metadata['previous_source_key']);
        $this->assertNotEmpty($video->metadata['edit_overlays'][1]['asset_key']);
        $this->assertTrue(Storage::disk('testlocal')->exists($video->metadata['edit_overlays'][1]['asset_key']));
