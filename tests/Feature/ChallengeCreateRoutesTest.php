...
class ChallengeCreateRoutesTest extends TestCase
{
    ...
    public function test_creator_store_endpoint_creates_an_approved_challenge_and_draft_endpoint_keeps_draft_status(): void
    {
        $creator = User::factory()->create();
        $approvedPayload = $this->payload('Approved creator challenge');
        $draftPayload = $this->payload('Draft creator challenge');

        $this->actingAs($creator)->withoutMiddleware()->postJson('/api/v1/creator/challenges', $approvedPayload)
            ->assertCreated();

        $this->actingAs($creator)->withoutMiddleware()->postJson('/api/v1/creator/challenges/draft', $draftPayload)
            ->assertCreated();

        $approvedChallenge = Challenge::query()->where('title', 'Approved creator challenge')->firstOrFail();
        $draftChallenge = Challenge::query()->where('title', 'Draft creator challenge')->firstOrFail();

        $this->assertSame(ChallengeStatus::Approved, $approvedChallenge->status);
    ...
    private function payload(string $title): array
    {
        return [
            ...
            'submission_starts_at' => now()->subHour(),
            'submission_ends_at' => now()->addWeek(),
